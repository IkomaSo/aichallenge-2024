...
TRAJ_NODE_NAME="/planning/scenario_planning/clothoid_traj_publisher"
...
send-keys "$AIC_WORKSPACE_CD_CMD" C-m
send-keys "$SOURCE_CMD" C-m
send-keys "./change_param.bash $TRAJ_NODE_NAME max_centripetal_acc cent_acc" C-m

select-pane -t 2
send-keys "set -x" C-m
send-keys "$AIC_WORKSPACE_CD_CMD" C-m
send-keys "$SOURCE_CMD" C-m
send-keys "./change_param.bash $TRAJ_NODE_NAME wheel_base wheel_base" C-m
